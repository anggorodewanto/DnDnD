...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .wizard-steps .step { padding: 0.5rem 1rem; background: #16213e; border-radius: 6px; border: 1px solid #0f3460; color: #999; }
    ...
    <div class="main">
        ...
        <div id="step-3" class="section">
            ...
            <div class="actions">
                ...
                <button class="btn btn-primary" onclick="goToStep(4)">Next: Review</button>
            </div>
        </div>
    ...
    fetch('/dashboard/api/characters/preview', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data)})
        ...
        .then(function(stats){
            ...
            document.getElementById('preview-stats').innerHTML =
                stat('HP', stats.hp_max) + stat('AC', stats.ac) +
            ...
            if (stats.saves) {
                ['str','dex','con','int','wis','cha'].forEach(function(ab){
                    ...
                    document.getElementById('preview-stats').innerHTML += stat(ab.toUpperCase() + ' Save', sign + v);
                });
            }
        });
}
...
function submitCharacter() {
    var data = gatherData();
    var payload = {campaign_id: new URLSearchParams(location.search).get('campaign_id') || '', name: data.name, race: data.race, background: data.background, classes: data.classes, ability_scores: data.ability_scores};
