...
// (errors moved to error_log). Service.CreateActionLog forwards the input
// UUIDs verbatim to the sqlc params struct.
func TestService_CreateActionLog_PassesUUIDsThrough(t *testing.T) {
	store := defaultMockStore()
	...
	require.NoError(t, err)
	assert.Equal(t, turnID, captured.TurnID)
	assert.Equal(t, encID, captured.EncounterID)
	assert.Equal(t, actorID, captured.ActorID)
}
