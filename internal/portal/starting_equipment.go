...
// BackgroundEquipmentPack returns the guaranteed starting equipment for a
// background. The table (backgroundEquipmentByID) is GENERATED from the
// canonical portal/svelte/src/lib/backgrounds.json shared with the Svelte
// builder — see backgrounds_gen.go / generate.go. Slugs are kebab-case.
func BackgroundEquipmentPack(background string) []string {
	return backgroundEquipmentByID[strings.ToLower(background)]
}
