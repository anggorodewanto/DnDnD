...
| STEP-005 | *(next — pick via QnA)* | `TODO` | — | Candidates: `/attack` (combat journey, one-shot — needs a "seed active combat/initiative" precondition) · `/cast` (reuses click model; touches concentration/slots flagged deferred) · combat initiative seeding (backlog #7). |
