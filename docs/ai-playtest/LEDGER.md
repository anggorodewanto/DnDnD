...
| STEP-007 | *(next — pick via QnA)* | `TODO` | — | Candidates: `/cast` (reuses dice infra; concentration/slots) · `/done` end-turn (turn-cycle, no dice) · combat initiative seeding · **structural guard**: one table-driven test asserting every public attack-producing `Service` method writes `UpdateCombatantHP` on a hit (prevents a 6th method re-introducing the STEP-005/006 bug class). |
