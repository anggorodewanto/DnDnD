...
| STEP-004 | *(next — pick via QnA)* | `TODO` | — | Candidates: `/attack` (combat journey, one-shot) · `/cast` (reuses click model) · `/setup` **admin-path** + rejections (needs harness `Member.Permissions` support — see STEP-003 deferred). |
...
| 1 | DM runs `/setup` (build channel structure) | inject `/setup` interaction | ✅ STEP-003 (DM happy path) |
