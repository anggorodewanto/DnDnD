...
- [x] **Phase 104b: Encounter Publisher Fan-out & Combat Store Adapter Cleanup**
  - Scope: Phase 104 wired `dashboard.Publisher` into `combat.Service` only. Extend the same `EncounterPublisher` injection pattern (`SetPublisher` + post-commit `publish()` hook, nil/error tolerant) to every other service whose mutations can affect an active encounter's combatant state mid-combat: `rest.Service` (HP/hit dice/spell slot restoration on short/long rest), `inventory.Service` (equip/unequip changing AC, ammunition consumption), `magicitem.Service` (attunement, active abilities altering stats), `levelup.Service` (mid-encounter level-up is rare but mutates HP/AC). For each service, after a successful DB-committing mutation that touches an encounter-resident combatant, call `publisher.PublishEncounterSnapshot(ctx, encounterID)`. Wire the real `dashboard.Publisher` into each service in `cmd/dndnd/main.go`. Skip services that only touch out-of-combat state (`shops`, `loot` post-encounter, `characteroverview`, narration/template/messageplayer).
  - Also: consolidate `cmd/dndnd/combat_store_adapter.go` with the test-only `testStoreAdapter` in `internal/combat/integration_test.go`. The cleanest fix is to align `refdata.Queries.UpdateCharacterInventory` and `UpdateCharacterGold` to sqlc's struct-params convention so `*refdata.Queries` directly satisfies `combat.Store` and the adapter can be deleted entirely. If that proves too invasive, promote a single exported `combat.NewStoreAdapter(*refdata.Queries)` so both tests and `main.go` reuse it.
...
- [x] **Phase 105b: Discord Handler Wiring in `main.go`**
...
- [x] **Phase 118c: sqlc Drift Reconciliation (Phase 118b Follow-up)**
  - Scope: Reconcile the drift between `db/migrations/` and the committed sqlc-generated layer (`internal/refdata/*.sql.go`, `internal/refdata/models.go`). During Phase 118b the implementer observed that running `sqlc generate` from scratch regenerates `action_log.sql.go`, `combatants.sql.go`, and `models.go` with nullable-column changes that don't match the currently committed code — meaning a prior phase shipped a migration without re-running `sqlc generate`. Tasks: (1) audit which migrations introduced the nullable changes (specifically `action_log.turn_id`, `action_log.encounter_id`, `action_log.actor_id`, plus several `combatants` columns), (2) run `sqlc generate` cleanly and commit the regenerated files, (3) fix every call site whose Go types changed (likely `sql.NullX` widening on params/rows — search for `action_log` and `combatants` consumers in `internal/combat`, `internal/errorlog`, `cmd/dndnd`), (4) add a CI guard that fails when the working tree is dirty after `sqlc generate` so this cannot drift again. The CI guard is the load-bearing deliverable — without it Phase 118c will recur.
  - Depends on: Phase 118b
  - Done when: `sqlc generate` produces a clean diff against `HEAD`; all consumers of the regenerated types compile and tests pass; `make cover-check` green; CI runs `sqlc generate && git diff --exit-code` (or equivalent) and fails on drift.

- [x] **Phase 118: Concentration Cleanup Integration (Phase 61 Follow-up)**
  - Scope: Wire `BreakConcentration` (internal/combat/concentration.go) into the production paths it currently isn't called from. Trigger points: damage-induced concentration save failure (combat damage pipeline), incapacitation (stunned/paralyzed/unconscious/petrified condition application), unconscious at 0 HP, Silence zone entry for V/S concentration spells, casting a new concentration spell while already concentrating, and voluntary drop via dashboard. On break, invoke the cleanup callback that (a) removes all spell-sourced conditions on every combatant in the encounter whose `SourceCombatantID` + `SourceSpell` match the dropped concentration spell (generalize Phase 113's `breakInvisibilityAndPersist` into a `removeSpellSourcedConditions(casterID, spellID)` helper), (b) removes `encounter_zones` whose `source_combatant_id` + `source_spell` match (Phase 71's hook already exists; verify it fires), (c) dismisses concentration-linked summons (Phase 80's `BreakConcentrationAndDismissSummons` — currently the only caller). Post a single consolidated `#combat-log` line: "💨 [Caster] lost concentration on [Spell] — effects ended on N targets." Covers Phase 113's Invisibility/Greater Invisibility gap and every other concentration-applied condition (Hold Person/Monster paralyzed, Web restrained, Crown of Madness charmed, Bless, Bane, etc.).
...
- [x] **Phase 120a: E2E Scenario Backfill — Real Registration, Movement, Loot**
  - Scope: Phase 120 stood up the harness with three substituted scenarios because the spec-named commands were still stubs at boot time: (1) `/register → DM approve → welcome` was substituted with `/status` because `cmd/dndnd/main.go` calls `discord.NewCommandRouter(bot, nil)` so `RegistrationDeps` is never wired and `/register` falls through to the stub handler; (2) `/move` was substituted with `/distance` because the production move handler requires a Tiled-parsed map beyond the harness's minimal seed; (3) `/loot` was substituted with `/rest` because `/loot`, `/inventory`, `/give`, `/attune`, `/unattune`, `/equip`, `/character` are stubbed in `attachPhase105Handlers`. This phase backfills the three real scenarios. Components: (a) wire `RegistrationDeps` into `NewCommandRouter` in `cmd/dndnd/main.go` so `/register`, `/import`, `/create-character` route to their existing handlers in `internal/registration`; extend the e2e harness with a `SeedDMApproval` helper (issues the dashboard's approval mutation directly) and replace `TestE2E_StatusScenario` with `TestE2E_RegistrationScenario` that drives `/register → DM approve → welcome message in the player's DM + `player_characters.status='approved'`; (b) extend `e2eHarness.SeedMap` to load a real Tiled `.tmj` fixture (small 10x10 grid with one wall) so the production `/move` handler's terrain/pathfinding/cover code paths execute; replace `TestE2E_DistanceScenario` with `TestE2E_MovementScenario` that drives `/move A1` → confirm component → `/done` and asserts (i) the actor's `combatants.position_col/position_row` updated, (ii) `turns.movement_remaining_ft` decremented, (iii) `#combat-log` posts the move + turn-end lines in order; (c) wire whichever `/loot` / `/inventory` / `/give` handlers exist (or land prerequisite implementation if any of them are still stubs at this phase's start — flag and stop) into `attachPhase105Handlers`; replace `TestE2E_RestScenario` with `TestE2E_LootScenario` that drives DM-places-loot → player `/loot claim <id>` → asserts `characters.inventory` JSONB gained the item and `#the-story` posts the claim line. Keep `TestE2E_SaveScenario` and `TestE2E_RecapEmptyScenario` unchanged. Net result: 5 scenarios still, but every one matches the spec's named flow.
  - Depends on: Phase 14 (registration), Phase 30 (grid movement), Phase 86 (item picker) or whichever phase wires `/loot`, Phase 120
  - Done when: `make e2e` still runs ≥5 scenarios green, the three substituted scenarios are replaced by the spec-named `/register`, `/move`, `/loot` flows, the deterministic-transcript drift detection covers the new scenarios, and the harness no longer stores any forward-looking dead fields introduced for substitutes.

- [ ] **Phase 121: Interactive Playtest Harness (Human DM + AI Player)**
  ...
  - Iterations:
    - [x] **121.1 — Quickstart Doc (`docs/playtest-quickstart.md`)**
    ...
    - [x] **121.2 — Player-Agent CLI Skeleton (`cmd/playtest-player/`)**
      - Implementation note: Discord bots cannot invoke slash commands as users (`InteractionCreate` is read-only from the bot side — platform constraint, not a missing feature). The REPL parses + validates each command and prints a "PASTE THIS" line for the human operator to paste into Discord. The recorder still captures both halves because the bot's responses flow back through the gateway.
    ...
    - [x] **121.3 — Transcript Recorder + Replay Bridge**
      - Implementation note: the loader lives in `internal/playtest/replay.go` (pure unit-tested) plus `cmd/dndnd/e2e_replay_test.go` (build tag `e2e`) for the harness wiring. `internal/e2e` does not exist as a directory; the replay bridge sits next to the harness instead.
    ...
    - [ ] **121.4 — Playtest Checklist Doc (`docs/playtest-checklist.md`)**
      ...
      - Status: doc merged with all 11 scenarios documented. Real-session transcripts beyond the smoke `sample.jsonl` deferred until first live playtest — they require a real Discord session or substantial harness fixture work to produce authentic data. Each scenario has a `Status: pending` marker that flips to `captured` as transcripts land.
    - [x] **121.5 — Simplify + Polish**
      ...
      - Implementation note: no `README.md` exists at the repo root, so the pointer landed in `CLAUDE.md` (the de-facto repo-level doc). Quickstart re-timing deferred to first live playtest session.
...
| Player Portal (Web) | Phase 91a, 91b, 91c, 92 |
