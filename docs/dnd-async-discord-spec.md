...
All notifications are Discord-only — no email, SMS, or webhook fallback. The three-tiered escalation system (50% nudge → 75% tactical summary → 100% DM decision prompt) provides multiple chances to respond before any turn is lost. DMs can extend timers or act on a player's behalf. Players experiencing missed notifications should check their Discord notification settings and mobile app configuration.
...
- **Action Resolver** — view `#dm-queue` items, apply outcomes with a click. Items are listed in **chronological order** (oldest first) with no priority sorting or filtering. The DM works through items top-to-bottom; the pending-count badges on encounter tabs provide sufficient cross-encounter prioritization. Clicking a queue item **expands it inline**, revealing resolution controls: a text field for the outcome narrative, and buttons for common mechanical effects (apply damage, add/remove conditions, move a token). The DM resolves without leaving the queue list, keeping surrounding items visible for context
...
- **Narrate Panel** — a rich narration editor for posting to `#the-story` from the dashboard. Accessible from the sidebar, the Campaign Home "Narrate" button, or the mobile-lite view. Features:
  - **Rich text editor** with Discord-flavored markdown preview — bold, italic, blockquotes, and a dedicated "read-aloud" boxed-text format (rendered as a Discord embed with a left border accent)
  - **Image attachments** — attach images (scene art, NPC portraits, handouts) that are uploaded alongside the narration message. Images are stored in the Asset Library for reuse
  - **Narration templates** — DM can save and reuse narration templates for common scene types: room descriptions, NPC introductions, combat transitions, travel montages, and custom categories. Templates support placeholder tokens (e.g., `{player}`, `{location}`) that the DM fills in before posting. Templates are campaign-scoped and managed from the Narrate Panel
  - **Template library** — browse, search, edit, duplicate, and delete saved templates. Each template has a name, category, and body
  - **Preview** — live preview of how the message will appear in Discord before posting
  - **Post history** — a scrollable log of recent narrations posted from the dashboard, so the DM can reference or re-post previous narrations
...
2. **JSONB for flexible data** — ability scores, features, spell slots, and inventory use JSONB columns rather than normalized tables. This matches the nested structure of 5e character data and avoids dozens of join tables for rarely-queried data. Combat-critical fields (HP, AC, position) remain as typed columns for indexed queries. **JSONB columns are not GIN-indexed.** All combat queries are scoped by encounter (typically 5–15 combatants), so JSONB filtering happens in application code after loading the encounter's combatant set. For example, checking "is this combatant restrained?" iterates the in-memory `conditions` slice rather than issuing a `@>` containment query. This avoids GIN write overhead on every condition change, spell slot deduction, and feature use — all of which occur frequently during combat. The same pattern applies to `characters.feature_uses`, `characters.spell_slots`, and `characters.features`: load by character ID (B-tree indexed), filter in Go.
