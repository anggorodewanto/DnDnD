...
_Last updated: 2026-07-18 — **SESH, the deep market — ⚔️ COMBAT LIVE in Sabinnet's reading room (Round 4, encounter
   ...
   **ledger** (full scatter-routing + a who-read-whom column for Sesh this season), and the **prize** — **Sabinnet**,
   ...
   - **✅ COMBAT WON (07-17) — encounter `8431a89b` COMPLETED, both watchers dead, back out of combat.** The con collapsed
     ...
     **Steady Aim → Shortbow Sneak 21** → **coat DEAD, last enemy down.** DM **ended combat** (Confirm End; status
     ...
     - **➤ NOW — ROUND 4, Vale's turn (PC) OPEN:** full action + move + bonus. **Eldritch Blast is a cantrip (no pact slot needed)** — still her best line. **AWAITING her move-or-`/done` — do NOT act for her.** Sabinnet is **FREE + bloodied (8 HP secret)** — no longer paralyzed, so the auto-crit window is CLOSED. **She flees/pulls the alarm-cord the moment she gets to act (≤29 HP) — the kill window is NOW.** **Only 1 enemy left: Sabinnet (F1, free).** Enemy HP/AC SECRET.
