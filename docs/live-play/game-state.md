...
_Last updated: 2026-07-18 — **SESH, the deep market — ✅ COMBAT WON in Sabinnet's reading room — SABINNET CAPTURED ALIVE (non-lethal), aftermath now (encounter `95f98525` COMPLETED). Old header (encounter
...
➤ COMBAT WON — **SABINNET CAPTURED ALIVE. R4: Windreth CRIT 19 + CRIT 6, Forge CRIT 15 + CRIT 10 (paralyzed auto-crits) → Sabinnet 58→8; she then made her end-of-turn WIS re-save (22 vs 14) and broke free; Vale finished it with Eldritch Blast + Agonizing Blast → 0 HP. Vale had PRE-DECLARED non-lethal intent (to interrogate), so per the new DM house rule (ranged/spell subdue allowed on pre-declared intent — see [dm-rules.md](dm-rules.md)) Sabinnet is UNCONSCIOUS + STABLE, not dead. All 3 enemies down; encounter ENDED (End Combat, HP carried back: WI 31/31, VA 31/31, FO 9/41). ➤ NOW — AFTERMATH in the reading room (out of combat): Sabinnet captured/senseless behind her desk, the pale rod fallen, the alarm-cord never pulled. Party can secure her, search the desk/ledgers/glass/papers, or wake + question her; Windreth still carries THE SEAL (may fit this desk). Awaiting the party in #in-character / their `/check` rolls — don't act for them.** [R3 history:] Vale cast Hold Person on Sabinnet; DM resolved the enemy WIS save (10 vs DC 14, FAIL) → SABINNET PARALYZED (save-ends WIS DC 14). Then Windreth into a HELD boss (advantage on all attacks; melee-within-5ft = AUTO-CRIT; Forge already adjacent G2). Forge is at 9/41 after Sabinnet's R3 Mind Lance (11 psychic).** Windreth (R3) moved C9→G5 + Cunning-Action Hide (Stealth 23, hidden) + Shortsword (adv, HIT, +Sneak, 7) + Dagger NAT-20 crit (5) → **HC2 to 4 HP (SURVIVED)**; **Forge's action handaxe (5+Rage) then dropped HC2 → DEAD (his kill)**. **BOTH HOUSECARLS DEAD — only Sabinnet left (58/58, F1, untouched).** Sabinnet flees/alarms at HP ≤29. ⚠ CORRECTION (07-18): an earlier note wrongly credited Windreth's crit with the HC2 kill — the crit left HC2 at 4 HP; **FORGE's action-attack killed it**. Forge's off-hand BONUS handaxe then whiffed the already-dead guard; per Forge's request the wasted bonus was **refunded** (one-time SQL `bonus_action_used=false`, user-approved) — proper dashboard `restore-bonus-action` endpoint being built. ⚠ Windreth double-spent his bonus (Hide + off-hand dagger) — engine allowed it; mook already doomed, left as-is (engine gap, not reversed).** The prior watcher fight (`8431a89b`) is COMPLETED (both watchers down); that history is recapped below.** The morning con collapsed into a **Rage door-breach** (Forge,
   ...
   **ledger** (full scatter-routing + a who-read-whom column for Sesh this season), and the **prize** — **Sabinnet**,
   ...
   - **✅ COMBAT WON (07-17) — encounter `8431a89b` COMPLETED, both watchers dead, back out of combat.** The con collapsed
     ...
     **Steady Aim → Shortbow Sneak 21** → **coat DEAD, last enemy down.** DM **ended combat** (Confirm End; status
     ...
     - **✅ ROUND 4 (1) Windreth (PC).** Into the PARALYZED boss (auto-crits within 5ft): `/attack` Shortsword **CRIT 19** + Dagger **CRIT 6** → Sabinnet 58→33. (Windreth now at **E2**.)
     - **✅ ROUND 4 (2) Forge (PC).** `/attack` Handaxe **CRIT 15** + Handaxe **CRIT 10** (incl. +2 Rage) → Sabinnet 33→**8 HP, BLOODIED**. (Forge **G2**, raging.)
     - **✅ ROUND 4 (3) Sabinnet (DM-run enemy turn).** Ran her turn via sanctioned in-page `POST /api/combat/{enc}/enemy-turn` with **empty steps** (paralyzed = incapacitated, no actions); engine auto-rolled her end-of-turn WIS re-save = **22 vs DC 14 → SUCCESS → paralysis ENDS, Vale's Hold Person concentration DROPS** (`ExecuteEnemyTurn` → `rollNPCEndOfTurnResave`). Then `POST …/advance-turn` → Vale. Break-free beat posted #the-story (3:06 PM 07-18).
     - **✅ ROUND 4 (4) Vale (PC).** `/cast` **Eldritch Blast + Agonizing Blast** on the freed Sabinnet → **0 HP.** Vale had pre-declared *"defeat non-lethally to interrogate,"* so per the new house rule (pre-declared intent lets a ranged/spell hit subdue — [dm-rules.md](dm-rules.md)) **Sabinnet is CAPTURED ALIVE (unconscious + stable), not dead.**
     - **✅ COMBAT WON — encounter `95f98525` ENDED (End Combat).** All 3 enemies down. Concentration dropped, HP carried back to sheets: **WI 31/31, VA 31/31, FO 9/41** (Forge leaves the fight badly hurt). Capture beat posted #the-story (3:16 PM 07-18).
     - **➤ NOW — AFTERMATH (out of combat), Sabinnet's reading room.** Sabinnet senseless behind her desk; the pale **rod fell out of reach**; the **alarm-cord was never pulled** (no reinforcements alerted... yet). The room is full of paper, ledgers, and glass cases. Party options: **secure/bind her**, **search the room** (`/check skill:investigation`, `/check skill:perception`), **wake + interrogate** (`/check skill:intimidation` / `skill:persuasion`; she's the sealed-name Reader-under-glass — the whole reason they came). **Windreth still carries THE SEAL** (`identified:false`) — this may be the desk it belongs to. **Also outstanding:** Forge is at 9/41 (short rest / heal candidate); his old thrown handaxe is still in the dead housecarl at F5 (trivial post-combat pickup). Do NOT act for the PCs — await their RP / rolls.
