...
1. **▶ THE PARTY HAS CROSSED PAST THE COLD VAULT — out of combat, at the lip of a vast buried gallery in the deeper
   dark.** The Cold Vault fight is long won (keeper `446dce33-…` destroyed, `status=completed`); the shrine find is
   resolved; both PCs short-rested (Forge's d12 rest now works — ISSUE-051 fixed) and **crossed the far throat into new,
   unprepped territory.** DM narrated the crossing (#the-story read-aloud, 07-01): a **buried gallery of the faceless
   god** — eyeless ovals worked into the walls, unnamed standing shapes on the floor, the pried idol's drag-mark ending
   somewhere out in a dark too big for the lantern, and **something out there noticing the light** (a slow turning of
   cold attention); Vale's patron pull points **dead ahead**. **Spotlight is on the players — await their next declared
   action in the gallery (advance / light it / stealth / call out / examine the shapes); never act, roll, or decide for
   them** (per [`dm-rules.md`](dm-rules.md)). **If it turns into a fight, build the map + encounter live** — no board is
   prepped past the Cold Vault (cold-vault design keeps reserve **Zombies**; scale per [`big-party.md`](big-party.md)).
   The old cleared-vault framing + shrine-find breadcrumb below is now **history** (all ✓), kept for reference.
