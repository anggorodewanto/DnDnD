...
- [x] 28. **Map readability.** — Resolved: No special zoom commands. Default tile size set to 48px (reduced to 32px only for large maps hitting Discord's 8MB limit). Images are large enough for Discord's native pinch-to-zoom on mobile.
...
- [x] 62. **Disconnection handling.** — Resolved: No special handling. The async-first 24h default timer covers client crashes; DM tools (extend/pause) handle edge cases. Discord presence is unreliable, so no bot-side disconnect detection.
...
- [x] 65. **No backup notification system.** — Resolved: Discord-only, no fallback. The three-tiered timeout escalation (50% nudge, 75% tactical summary, 100% DM decision prompt) plus DM override tools (extend, pause, roll for player) provide sufficient coverage. Players fix missed notifications via Discord settings.
...
- [x] 100. **Carrying capacity.** — Resolved: Not enforced. No weight or slot limits; DM controls item availability through loot/shops/rewards. Weight data on reference tables is informational only.
- [x] 101. **Languages.** — Resolved: Flavor only, no mechanical enforcement. Added `languages TEXT[]` to characters table (populated from race + class + background + feats). Shown on character cards and character sheet. DM controls comprehension via narration. DM Dashboard Character Overview includes a Party Languages summary showing language coverage across the party.
...
- [x] 107. **Campaign pause UX.** — Resolved: Announcement only. Bot posts pause/resume messages in `#the-story`. All commands remain functional; `paused` status is informational. DM manages expectations out-of-band.
