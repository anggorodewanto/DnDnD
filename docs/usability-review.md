...
**Legend:** 🔴 blocker · 🟠 major · 🟡 minor · 🔵 validation · Progress: **8 / 52**
...
- [ ] **T10** · 🟠 · Map token errors → 400/409/410 with "request a new link"; key localStorage draft by user+campaign · _Finding 4 ·a_
- [ ] **T11** · 🟠 · Hydrate builder from the player's pending / changes_requested character (or persist draft server-side) · _Finding 4 ·b_
- [ ] **T12** · 🟠 · Confirm-on-navigate when dirty + `beforeunload` handler (map editor & encounter builder) · _Finding 4 ·c_
...
- [ ] **T16** · 🟠 · Split `/help` reply via `SplitMessage`; stop discarding the `InteractionRespond` error · _Finding 8_
- [ ] **T17** · 🟠 · Seat PCs into `player` spawn zones at StartCombat (reuse `AssignPCsToSpawnZones`) · _Finding 9_
...
- [ ] **T21** · 🟠 · Encounter no-map dead-end: disable/validate the "-- No map --" option inline before save · _Campaign setup_
- [ ] **T22** · 🟠 · Rejection feedback in `/character` + channel/ephemeral fallback when DM send fails · _Player onboarding_
- [ ] **T23** · 🟠 · Add an out-of-combat branch to `CastHandler.Handle` (skip turn lookup, permit rituals/utility) · _Live play_
